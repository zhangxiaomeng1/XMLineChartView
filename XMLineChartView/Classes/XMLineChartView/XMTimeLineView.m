...
@interface XMTimeLineView()

@property (nonatomic,assign) CGFloat volumeWidth;
@property (nonatomic,strong) XMTimeDataset *dataset;
...
- (void)setCurrentDataMaxAndMin
{
    if (self.dataset.data.count > 0) {
        ...
        for (NSInteger i = 0; i < self.dataset.data.count; i++) {
            XMTimeLineEntity  * entity = [self.dataset.data objectAtIndex:i];
        ...
        self.maxPrice =((XMTimeLineEntity *)[self.dataset.data firstObject]).preClosePx + self.offsetMaxPrice;
        self.minPrice =((XMTimeLineEntity *)[self.dataset.data firstObject]).preClosePx - self.offsetMaxPrice;
...
- (void)drawTimeLine:(CGContextRef)context
{
    ...
    for (NSInteger i = 0 ; i< self.dataset.data.count; i ++) {
        ...
        if (i > 0){
            XMTimeLineEntity * lastEntity = [self.dataset.data objectAtIndex:i -1];
