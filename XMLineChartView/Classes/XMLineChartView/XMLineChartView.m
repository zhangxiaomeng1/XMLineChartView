...
@property (nonatomic,strong)XMLineDataSet * dataSet;
...
- (void)setCurrentDataMaxAndMin
{
    ...
    if (self.dataSet.data.count > 0) {
        ...
        for (NSInteger i = idx; i < self.startDrawIndex + self.countOfshowCandle && i < self.dataSet.data.count; i++) {
            XMLineEntity  * entity = [self.dataSet.data objectAtIndex:i];
...
- (void)drawCandle:(CGContextRef)context
{
    ...
    for (NSInteger i = idex ; i< self.dataSet.data.count; i ++) {
        ...
        if (open < close) {
            ...
            if (i > 1) {
                XMLineEntity * lastEntity = [self.dataSet.data objectAtIndex:i-1];
...
- (void)notifyDataSetChanged{
    ...
    [self setNeedsDisplay];
    NSLog(@"==startDrawIndex=%ld",self.startDrawIndex);
    self.startDrawIndex = self.dataSet.data.count - self.countOfshowCandle;
    NSLog(@"==startDrawIndex=%ld==%ld==%ld",self.startDrawIndex,self.dataSet.data.count,self.countOfshowCandle);
    
    if (self.dataSet.data.count >100 && _isStartPoint == NO) {
        self.startDrawIndex = 50;
    }
    NSLog(@"==END=%ld==%ld==%ld",self.startDrawIndex,self.dataSet.data.count,self.countOfshowCandle);
